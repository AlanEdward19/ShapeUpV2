...
- **Phase / Task**: Phase 1 (T1-T7), Phase 2 (T8-T13, T16), Phase 3 (T19-T24, redesenhada — AD-005) completas. AD-003 resolvido (AD-006) — T14, T15, T17, T18 migrados na sequência. **Todo o backlog de migração de controllers está completo.** Falta só Phase 4 (T25-T27, descomissionamento do legado `Group/Scope`).
- **Completed**:
  ...
  - Suite final: 339 unit tests + 233 integration tests, 0 falhas.
- **In-progress**: nenhum
- **Next step**: Phase 4 — T25 (remover `Group/Scope/UserGroup/GroupScope` + `RequireScopesAttribute.cs`). **Grep de confirmação rodado (2026-09-06)**: ainda sobra `RequireScopesAttribute` em 4 lugares fora do próprio módulo Authorization — `NotificationsController` (2 endpoints, `send_html`/`send_template`), `GetAuditLogsController` (1 endpoint — provavelmente `capability:platform.audit_logs.read`, mesma família do AD-006), `GymsController.GetAll`/`Create` (2 endpoints, deixados de propósito no T8 por não terem `gymId` na rota). T25 não é só "deletar Group/Scope" — precisa migrar esses 5 endpoints primeiro (ou decidir explicitamente mantê-los em outro mecanismo). Depois de T25: T26 (`ARCHITECTURE.md`/`README.md`), T27 (fechar RFC-001/`CURRENT_STATE_ASSESSMENT.md`).
- **Blockers**: nenhum bloqueante — só escopo adicional de T25 ainda não mapeado em tasks.md (ver acima).
