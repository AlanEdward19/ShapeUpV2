...
> ADENDO (antes da Fase 2 paralela): registro de Policy trocado de `AddPolicy` explícito por `CapabilityPolicyProvider` (`IAuthorizationPolicyProvider` dinâmico — qualquer policy `"capability:*"` vira `CapabilityRequirement` sem precisar editar `DependencyInjectionExtensions.cs`). Motivo: 8 tasks paralelas (T8-T15) editando o mesmo bloco de `AddPolicy` gerariam conflito de merge garantido. Com isso, cada controller migrado toca só o próprio arquivo.
